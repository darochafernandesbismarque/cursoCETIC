...
<body>
  ...
  if (isset($_POST['excluir'])) {
    ...
    $estoque    = mysqli_fetch_array(ejecutarSQL::consultar("SELECT sum(e.qnt_atual)as estoque 
            FROM  entrada e  
            where id_material='$id_material'"))[0];
  ...
  <div class="content-page-container full-reset custom-scroll-containers">
    ...
    if (isset($_POST['enviar'])) {
      ...
      $origem = empty(explode(",", $_POST['unidade'])[0]) ? '' : 'opm=' . explode(",", $_POST['unidade'])[0];

      $origem_desc = !empty(explode(",", $_POST['unidade'])[0]) == "" ? '' : explode(",", $_POST['unidade'])[1] . ' , ';
    } else {
      $datainic = strftime('%Y-%m-%d', strtotime('-15 days'));
      $datafim   = date("Y-m-d");
      $destino  = '';
      $origem   = 'opm=' . $opm_logado;
    }

    $qryentrada = "SELECT m.descricao,qnt,data,f.descricao as fornecedor,vl_uni,vl_total,nf,status,user, e.id,id_material FROM entrada e,material m,fornecedor f WHERE data BETWEEN '$datainic' AND '$datafim' $idMaterialEntrada  $fornecedor $tipoverba and e.id_material=m.id and $origem and id_fornecedor=f.id  order by e.id desc";
    ...
    $qrysaida = "SELECT * FROM saida s,material m, tabopm o WHERE data_saida BETWEEN '$datainic' AND '$datafim' and $origem $destino  $idMaterialSaida and s.id_material_saida=m.id and s.destino=o.cod_opm order by id_saida desc";

    $qrysaidaxls = "SELECT * FROM saida s,material m, tabopm o WHERE data_saida BETWEEN '$datainic' AND '$datafim' and $origem $destino  $idMaterialSaida and status<>2 and s.id_material_saida=m.id and s.destino=o.cod_opm order by id_saida desc";
    ...
    if ($contador       = mysqli_fetch_array(ejecutarSQL::consultar("SELECT sum(e.qnt) FROM entrada e,material m WHERE data BETWEEN '$datainic' AND '$datafim' $idMaterialEntrada and e.id_material=m.id and $origem and status<>2"))) {
    ...
    $qrygrafico = "SELECT sum(s.qnt_saida) as total, s.destino,o.abrev_opm FROM  saida as s,tabopm as o, material m WHERE s.destino in (SELECT destino FROM  saida where $origem ) and data_saida BETWEEN '$datainic' AND '$datafim' $destino  $idMaterialSaida  and s.destino=o.cod_opm and s.id_material_saida=m.id and s.status<>2 group by s.destino";
    // echo "string".$qrygrafico;             
    ?>
          ...
          <form method="post" action="<?php echo $idget; ?>">
            <div class="col-xs-12 col-sm-1">
              ...
              <div class="group-material">
                <select class="selectpicker form-control" data-size="20" data-live-search="true" name="unidade" required>

                  <button type="button" class="btn btn-success"> + </button>

                  <?php
                  echo '<option value="' . $opm_logado . ',PROPRIA">PROPRIA</option>';
                  if ($opm_logado == 164) {
            ...
            <div class="col-xs-12 col-sm-1">
              ...
              <div class="group-material">
                <select class="selectpicker form-control" data-size="20" data-live-search="true" name="destino" required>
                  ...
                  <?php
                  $queryopms = ejecutarSQL::consultar("SELECT * FROM opm where cod_opm>502 order by abrev_opm ");
                  while ($opms = mysqli_fetch_array($queryopms)) {
                    echo '<option value="' . $opms["cod_opm"] . ',' . $opms["abrev_opm"] . '" >' . $opms["abrev_opm"] . '</option>';
                  } ?>
                </select>
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
      <button class="btn" data-toggle="collapse" data-target="#filtros" title="Filtros" style="background-color:#FFFFFF;"><i class="glyphicon glyphicon-filter"></i> &nbsp;
        <label class="a"> <?php echo $origem_desc . ' ' . date("d-m", strtotime($datainic)) . "   a  " . date("d-m", strtotime($datafim)) . '  ' . $destino_desc . ' ' . $fornecedor_desc; ?></label> </button>
    </div>
    <br>
    <div class="container">
      <div class="page-header">
    ...
    <div>
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12  collapse multi-collapse text-center" id="entradas">
        ...
        <div class="table-responsive">
          <div>
            <?php
            $qrysaidaDabst = "SELECT data_saida,descricao,qnt_saida,valor_individual,val_total,documento,rg_retirada,rg_status,status FROM saida s,material m, tabopm o WHERE data_saida BETWEEN '$datainic' AND '$datafim' and opm=499 and destino='$opm_logado'  $idMaterialSaida and s.id_material_saida=m.id and s.destino=o.cod_opm  order by s.id_saida desc";

            $querysaidaDabst = ejecutarSQL::consultar($qrysaidaDabst);
                      ...
                      <div class="table-responsive">
                        <div class="div-table" style="margin:0 !important;<?php echo $style; ?>">
                          <div class="div-table-row div-table-row-list">
                            ...
                            <div class="div-table-cell" style="width: 10%;"><?php echo $saidastotal['val_total']; ?></div>
                            <div class="div-table-cell" style="width: 10%;"><?php echo $saidastotal['abrev_opm']; ?></div>
                            ...
                            <div class="div-table-cell " style="width:10%;">
                              <input class="btn btn-primary tooltips-general" type="button" style="width:100%" title="<?php
                                                                                                                      if ($saidastotal['status'] == 2) {
                                                                                                                        echo 'Exclída por ' . $saidastotal['rg_status'] . '&nbsp;&nbsp;Obs:&nbsp;' . $saidastotal['obs'];
                                                                                                                      }
                                                                                                                      ?>" value="<?php echo $saidastotal['m60']; ?>"></>
                            </div>
                          </div>
                        </div>
                      </div>
      ...
      <div class="modal fade" id="excluir" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
          <div class="col-md-8 col-md-offset-2  collapse multi-collapse" id="saidas-excluidas">
            ...
            <h1 class="all-tittles"><small>MODELOS 60 EXCLUÍDOS </small></h1>
            <div class="table-responsive">
              <div class="div-table" style="margin:0 !important;">
                <div class="div-table-row div-table-row-list" style="background-color:#DFF0D8; font-weight:bold;">
                  <div class="div-table-cell" style="width: 25%;">DATA MOD60</div>
                  <div class="div-table-cell" style="width: 15%;">LOGIN</div>
                  <div class="div-table-cell" style="width: 25%;">DESTINO</div>
                  <div class="div-table-cell" style="width: 15%;">RG EXCLUSÃO</div>

                  <div class="div-table-cell" style="width: 25%;">MOD60</div>
                </div>
              </div>
              <div style="max-height: 250px">
                <?php $qrysaida2 = "SELECT DISTINCT data_saida,user_saida, m60,rg_exclui,o.abrev_opm FROM saida s,tabopm o WHERE data_saida >='$datainic' AND data_saida<='$datafim' $destino $idMaterialSaida AND s.destino=o.cod_local and status=2 order by data_saida desc";
                ...
                if (mysqli_num_rows($querysaida2) >= 1) {
                  ...
                  while ($saidastotal2 = mysqli_fetch_assoc($querysaida2)) {
                    $total++;
                    $style  = $saidastotal2['status'] == 2 ? 'text-decoration: line-through;' : ''; ?>
                    <ul id="itemContainer" class="list-unstyled">
                      <li>
                        <div class="table-responsive">
                          <div class="div-table" style="margin:0 !important;<?php echo $style; ?>">
                            <div class="div-table-row div-table-row-list">
                              <div class="div-table-cell" style="width: 25%;"><b><?php echo date('d-m-Y', strtotime($saidastotal2['data_saida'])); ?></b></div>
                              <div class="div-table-cell" style="width: 15%;"><b><?php echo $saidastotal2['user_saida']; ?></b></div>
                              <div class="div-table-cell" style="width: 25%;"><b><?php echo $saidastotal2['abrev_opm']; ?></b></div>
                              <div class="div-table-cell" style="width: 15%;"><b><?php echo $saidastotal2['rg_exclui']; ?></b></div>

                              <div class="div-table-cell" style="width: 25%;"><a href="modelo60Opm.php?id=<?php echo $saidastotal2['m60']; ?>" target="_blank"><button class="btn btn-primary" type="button"><i class="glyphicon glyphicon-print"></i></>&nbsp;<?php echo $saidastotal2['m60']; ?></button></a></div>
                            </div>
                          </div>
                        </div>
                      </li>
                ...
                <?php }
                  ...
                  echo '<h2 class="text-center all-tittles">Não há registros de exclusões no Sistema</h2>';
                } ?>
              </div>
              <div class="div-table-row div-table-row-list" style="background-color:#DFF0D8; font-weight:bold;">
                <div class="div-table-cell" style="width: 30%;">TOTAL :</div>
                <div class="div-table-cell" style="width: 30%;"><?php echo $total; ?></div>
              </div>
            </div>
          </div>
